...
export const EventFlyerGeneratorModal: React.FC<Props> = ({ isOpen, event, onClose }) => {
  ...
  const handleGenerate = async () => {
    ...
    if (error) {
      console.error('generate-event-flyer invoke error:', error);
      if (error instanceof FunctionsHttpError) {
        try {
          const errBody = await error.context.json();
          console.error('generate-event-flyer edge response body:', errBody);
          const providerMessage =
            errBody?.provider?.error?.message ||
            errBody?.error ||
            'Could not generate flyer.';
          setErrorMsg(String(providerMessage));
          return;
        } catch (parseError) {
          console.error('Failed to parse edge error response:', parseError);
        }
      }
