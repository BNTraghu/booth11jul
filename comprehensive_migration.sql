...
-- Add all missing columns for venues and events tables

-- =====================================================
-- VENUES TABLE MIGRATION
-- =====================================================

-- Add amenities column (this was missing!)
...
-- Add image URL field if not exists
...
WHERE NOT EXISTS (
    ...
    WHERE table_name = 'venues' 
    AND column_name IN ('amenities', 'address_line1', 'address_standard', 'area_sq_ft', 'kind_of_space', 'latitude', 'longitude', 'formatted_address', 'description')
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'events' 
    AND column_name IN ('event_end_date', 'event_end_time', 'event_image_url')
);
